...
class Next_Revalidation {
    ...
    private $last_revalidation_key = 'next_revalidation_last_time';
    private $revalidation_cooldown = 1; // seconds between revalidations
    ...
    public function __construct() {
        ...
        add_action('update_option_contact_phone', array($this, 'on_contacts_change'), 10, 2);
        add_action('update_option_contact_email', array($this, 'on_contacts_change'), 10, 2);
        add_action('update_option_contact_address', array($this, 'on_contacts_change'), 10, 2);
        add_action('update_option_footer_description', array($this, 'on_contacts_change'), 10, 2);
    ...
    public function sanitize_settings($input) {
        ...
        if(isset($input['revalidation_cooldown'])) {
            ...
            $new_input['revalidation_cooldown'] = max(0, min(120, $cooldown)); // Between 0 and 120 seconds
        }
    ...
    public function revalidation_cooldown_callback() {
        $value = isset($this->options['revalidation_cooldown']) ? intval($this->options['revalidation_cooldown']) : 5;
        echo '<input type="number" min="0" max="120" id="revalidation_cooldown" name="' . $this->option_name . '[revalidation_cooldown]" value="' . $value . '" class="small-text" />';
        echo '<p class="description">Minimum seconds between revalidation requests (0-120). Use higher values for busy sites.</p>';
    }
    ...
    public function on_contacts_change($old_value, $value) {
    ...
    private function send_revalidation_request($content_type, $content_id = null) {
        ...
        // Implement per-content throttling to prevent duplicate requests
        $throttle_key = $this->last_revalidation_key . '_' . $content_type . '_' . ($content_id ?? 'all');
        $current_time = time();
        $last_revalidation = get_transient($throttle_key);
        
        if ($last_revalidation !== false && ($current_time - $last_revalidation) < $cooldown) {
        ...
        // Store current time in transient per content type/id
        set_transient($throttle_key, $current_time, $cooldown + 60);
