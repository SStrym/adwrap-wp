    ...
    public function healthTestLicense()
    {
        ...
        if (! $license instanceof License || ! $license->isValid()) {
            ...
            if (is_wp_error($response)) {
                $license = ($license instanceof License)
                    ? $license->checkFailed($response)
                    : License::fromError($response);
            } else {
        ...
        if (is_wp_error($response)) {
            wp_send_json_success([
                ...
                'description' => sprintf(
                    ...
                    esc_html(implode(', ', array_merge(
                        ['code' => $response->get_error_code()],
                        array_filter($response->get_error_data() ?? [])
                    )))
                ),
                'badge' => ['label' => 'Object Cache Pro', 'color' => 'red'],
                'status' => 'critical',
